...
resource "azurerm_container_app" "container_ai" {
  name                         = "${var.prefix}-${var.project}-azsc1-aca-${var.environment}-ai"
...
resource "azurerm_role_assignment" "container_ai_kv_access" {
  scope              = module.key_vault.resource_id
  role_definition_id = data.azurerm_role_definition.key_vault_secrets_user.id
  principal_id       = azurerm_container_app.container_ai.identity[0].principal_id
...
resource "azurerm_role_assignment" "container_ai_acr_pull" {
